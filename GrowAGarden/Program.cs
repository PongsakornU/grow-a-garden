...
class Program
{
    static async Task Main()
    {
        try
        {
            ...
            var discordWebhook = Environment.GetEnvironmentVariable("DISCORD_WEBHOOK_URL");

            if (string.IsNullOrEmpty(discordWebhook))
            {
                Console.WriteLine("⚠️ No Discord webhook found!");
                return;
            }
            ...
            var Seeds = new List<string> { "Romanesco", "Burning Bud", "Elder Strawberry", "Giant Pinecone", "Sugar Apple", "Ember Lily", "Beanstalk", "Mushroom", "Crimson Thorn"};
